...
# Example 2: Create transaction with negative amount
print_section "Example 2: Create Transaction (Negative Amount)"
...
# Example 3: Create transactions in different currencies
print_section "Example 3: Create Transactions (Multiple Currencies)"
...
# Example 4: Get transaction by ID
if [ -n "$TRANSACTION_ID" ] && [ "$TRANSACTION_ID" != "null" ]; then
    print_section "Example 4: Get Transaction by ID"
    print_request "GET /transactions/$TRANSACTION_ID"
    echo ""
    curl -s -X GET "$BASE_URL/transactions/$TRANSACTION_ID" | jq '.'
fi

# Example 5: List all transactions for user
print_section "Example 5: List All Transactions for User"
print_request "GET /transactions?user_id=demo_user_1"
echo ""
curl -s -X GET "$BASE_URL/transactions?user_id=demo_user_1" | jq '.'

# Example 6: List transactions for user and currency
print_section "Example 6: List Transactions (User + Currency)"
...
# Example 7: List transactions with pagination
print_section "Example 7: List Transactions (Pagination)"
