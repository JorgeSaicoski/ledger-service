...
- Request JSON:
  ...
  - `currency` (string, required, lowercase alphanumeric, underscores allowed) — e.g. `usd`, `brl`, `loyalty_points`
- Success: 201 Created
- Response JSON:
  - `id` (uuid, lowercase)
...
- Transaction:
  ...
  - `currency` TEXT NOT NULL (must be lowercase alphanumeric, underscores allowed)
...
- `currency` must be lowercase alphanumeric (underscores allowed), max length 32
