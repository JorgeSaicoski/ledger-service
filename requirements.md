...
- Request JSON:
  ...
  - `currency` (string, required, lowercase alphanumeric with underscores) — e.g. `usd`, `brl`, `loyalty_points`
- Success: 201 Created
- Response JSON:
  - `id` (uuid, lowercase)
...
- Transaction:
  ...
  - `currency` TEXT NOT NULL (must be lowercase alphanumeric with underscores)
...
- `currency` must be lowercase alphanumeric with underscores, max length 32
